...
matplotlib.use("Agg")  # server-side chart rendering
...
def make_daily_revenue_chart(df: pd.DataFrame) -> str:
    ...
    filename = f"gunluk_ciro_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
...
def summarize(df: pd.DataFrame) -> dict:
    ...
    # JSON'a basılabilmesi için (date objesi -> string)
...
# -------------------------
# IMPORTANT: initialize DB on import (Gunicorn/Render)
# -------------------------
init_db()
